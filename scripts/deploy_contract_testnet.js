/**
 * Stacks contract deployment script with detailed error logging
 * Deploys a Clarity contract to Stacks Testnet
 */
const { makeContractDeploy, broadcastTransaction, standardPrincipalCV, ClarityType } = require('@stacks/transactions');
const fs = require('fs');

/**
 * Deploy contract to testnet with robust error handling
 */
async function deployContract() {
  // Configuration
  const privateKey = 'ed491076ac8f2446f5840a241cc16382207e4984b1ac7268ba39e5ff96672a7c';
  const contractName = 'microtasks-v4'; // Using a unique name to avoid conflicts
  const contractPath = './contracts/microtasks.clar';
  const fee = 50000; // 0.05 STX
  const nonce = 2; // Increment by 1 from your last transaction

  // Print deployment info
  console.log('======== DEPLOYMENT CONFIGURATION ========');
  console.log(`Contract name: ${contractName}`);
  console.log(`Network: testnet`);
  console.log(`Fee: ${fee} microSTX`);
