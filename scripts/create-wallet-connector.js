...
(function() {
  ...
  window.stacksConnect = {
    ...
    openConnect: async function(options) {
      ...
      // Format for Leather Wallet (following correct docs)
      const appParamsBase64 = btoa(JSON.stringify({
        appDetails: appDetails,
        redirect: redirectUrl
      }));
      
      // Use the correct URL for Leather Wallet
      const authUrl = \`https://app.leather.io/authentication?authRequest=\${appParamsBase64}\`;
    ...
    openContractCall: async function(options) {
      ...
      const txUrl = \`https://app.leather.io/transaction?contractCall=\${txParamsBase64}\`;
