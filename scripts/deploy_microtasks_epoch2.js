/**
 * Deploy Microtasks Contract with Epoch 2.0
 * This script uses the correctly configured contract for the testnet's current epoch
 */
const { makeContractDeploy, broadcastTransaction } = require('@stacks/transactions');
const fs = require('fs');

/**
 * Deploy with correct epoch settings
 */
async function deployContract() {
  // Configuration
  const privateKey = 'ed491076ac8f2446f5840a241cc16382207e4984b1ac7268ba39e5ff96672a7c';
  const contractName = 'microtasks-epoch2'; // Using the correct epoch contract
  const contractPath = './contracts/microtasks-epoch2.clar';
  const fee = 200000; // 0.2 STX
  const nonce = 8; // Updated based on latest account check
  
  // Print deployment info
  console.log('======== MICROTASKS EPOCH 2.0 DEPLOYMENT ========');
